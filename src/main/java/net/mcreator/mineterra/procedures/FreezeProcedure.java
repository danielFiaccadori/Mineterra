...
public class FreezeProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if (entity instanceof LivingEntity _livEnt1 && _livEnt1.hasEffect(MineterraModMobEffects.FROZEN.get())) {
			if (entity instanceof Player) {
				if (_evt.getRenderer() instanceof PlayerRenderer) {
					if (_evt instanceof RenderLivingEvent.Pre) {
						// _evt.setCanceled(true);
					}
					new com.kleiders.kleidersplayerrenderer.KleidersPlayerRenderer(context, new ResourceLocation("mineterra:textures/entities/icetexture.png"), new Modelice(context.bakeLayer(Modelice.LAYER_LOCATION)))
				...
				if (!(_evt.getRenderer() instanceof com.kleiders.kleidersplayerrenderer.KleidersEntityRenderer)) {
					...
					new com.kleiders.kleidersplayerrenderer.KleidersEntityRenderer(context, new ResourceLocation("mineterra:textures/entities/icetexture.png"), new Modelice(context.bakeLayer(Modelice.LAYER_LOCATION))).render((Mob) _evt.getEntity(),
